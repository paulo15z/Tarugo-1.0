...
def gerar_xls_roteiro(pecas: List[PecaOperacional]) -> bytes:
    ...
    for p in pecas:
        row = {
            ...
            "BORDA_FACE_FRENTE": p.bordas.get("top", {}).get("nome", "") if p.bordas else "",
            "BORDA_FACE_TRASEIRA": p.bordas.get("bottom", {}).get("nome", "") if p.bordas else "",
            "BORDA_FACE_LE": p.bordas.get("left", {}).get("nome", "") if p.bordas else "",
            "BORDA_FACE_LD": p.bordas.get("right", {}).get("nome", "") if p.bordas else "",
